...
function detectIntent(message) {
  ...
  const navPatterns = [
    ...
    /(?:route|path|direction|way)\s+(?:from|between)\s+(.+?)\s+(?:to|and)\s+(.+)/i,
    /(?:shortest|fastest|best)\s+(?:route|path|way)\s+(?:from|between)\s+(.+?)\s+(?:to|and)\s+(.+)/i,
    /(?:from)\s+(.+?)\s+to\s+(.+)/i,
  ];
  for (const pattern of navPatterns) {
    const match = msg.match(pattern);
    if (match) return { type: 'navigation', from: match[1].trim(), to: match[2].trim() };
  }
  const nearestPatterns = [/(?:nearest|closest|nearby)\s+(.+)/i, /(?:where\s+is\s+the\s+(?:nearest|closest))\s+(.+)/i];
  ...
  const locationPatterns = [/(?:where\s+is|locate|find|show\s+me|take\s+me\s+to)\s+(.+)/i, /(?:location\s+of)\s+(.+)/i];
...
function processChat(message) {
  ...
  switch (intent.type) {
    case 'spatial': {
      ...
      if (!target) response.text = `❌ I couldn't find "${intent.target}" on campus.`;
      else if (!reference) response.text = `❌ I couldn't find "${intent.reference}" on campus.`;
      else if (target.id === reference.id) response.text = `You asked for the distance between ${target.name} and itself. The distance is 0 meters!`;
      else {
        ...
        if (intent.subtype === 'direction') response.text = `🧭 **${target.name}** is located to the **${dir}** of **${reference.name}** (approx. ${distStr} away).`;
        else response.text = `📏 The distance between **${target.name}** and **${reference.name}** is approximately **${distStr}** (in the **${dir}** direction).`;
    ...
    case 'navigation': {
      ...
      if (!from) response.text = `❌ I couldn't find "${intent.from}" on campus.`;
      else if (!to) response.text = `❌ I couldn't find "${intent.to}" on campus.`;
      else {
        const route = dijkstra(from.id, to.id);
        if (route) {
          const names = route.waypoints.map((w) => w.name).join(' → ');
          response.text = `🗺️ **Route from ${from.name} to ${to.name}:**\n\n📍 ${names}\n\n📏 Total distance: ~${route.distance} meters\n⏱️ Estimated walk time: ~${Math.ceil(route.distance / 80)} minutes`;
          response.actions = [{ type: 'route', waypoints: route.waypoints, from, to }];
          response.buildings = [from, to];
        } else response.text = `I couldn't find a walking route between ${from.name} and ${to.name}.`;
      }
      break;
    }
    case 'nearest': {
      ...
      if (targetType) {
        ...
        } else response.text = `I couldn't find anything matching "${query}" on campus.`;
      }
      break;
    }
    case 'location': {
      ...
      if (building) {
        ...
        if (building.amenities?.length) response.text += `\n🛠️ Amenities: ${building.amenities.join(', ')}`;
        response.actions = [{ type: 'zoom', building }];
        response.buildings = [building];
      } else response.text = `❌ I couldn't find "${intent.query}" on campus.`;
      break;
    }
    case 'info': {
      const building = findBuilding(intent.query);
    ...
    default: {
      ...
      if (message.toLowerCase().includes('chitkara') && !message.toLowerCase().includes('where')) {
        ...
        if (building) {
          ...
          response.actions = [{ type: 'zoom', building }];
          response.buildings = [building];
        } else response.text = `🤔 I'm not sure about that. Try asking me:\n• "Where is [building name]?"\n• "Route from [A] to [B]"`;
      }
    }
  }
  return response;
